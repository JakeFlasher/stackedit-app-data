...
To achieve a desired instruction reduction rate while preserving critical change points, we need a fine-grained algorithm to help us filter those instructions that are supposed to contribute very little to cumulative IPC change as well as various cache performance metrics. Therefore, we employ an adaptive window size calculation  using  binary search. The goal is to determine an optimal window size \( W \) that, when applied on each change point fr, results in a number of preserved  instructions $\(D\)$ closest to the desired number within an acceptable tolerance $\( \epsilon \)$, given an original trace file $\( T \)$, detected change points $\( \mathcal{C} \)$ and a desired instruction reduction rate $\( r \)$.
  . 
...
- **Variables:**
  ...
  - : Tolerance (e.g., \( 0.05 \) for \( \pm5\% \) tolerance).
  -  Desired number of instructions to preserve, calculated as \( D = T (1 - r) \).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExOTg1OTgyOThdfQ==
-->